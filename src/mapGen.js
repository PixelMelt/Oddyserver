...
var II0I0 = function() {
    ...
    function t(t, e, i) {
    var s, l, n, a, o, r;
    for (this.seed = null != t ? t : Math.random(), this.size = null != e ? e : 20, this.mode = i, this.random = new lO1ll(this.seed), this.I1l01 = 1e5 * this.random.next(), this.I01IO = 1e5 * this.random.next(), this.a = 13971, this.c = 12345, this.mod = 1 << 30, this.inv = 1 / this.mod, this.l10Ol = this.mod - 1, this.IlO1O = .5, this.OlO0O = .5, this.lO0l1 = 5, this.O1lIO = 1 + this.random.OOOl1(8), this.fx = this.O1lIO / this.size, this.fy = this.O1lIO / this.size, this.fx = (2 + 8 * this.random.next()) / (2 * this.size), this.fy = (2 + 8 * this.random.next()) / (2 * this.size), this.map = new II0I0(this.random.next()), this.Ol0I0 = 2 * this.size, this.buffer = [], this.custom_map = null, s = n = 0, o = this.Ol0I0 - 1; n <= o; s = n += 1)
...
], createMapByID = function(size,id,gamemode,graphonly) {
    if(graphonly == true){
        edge = size/2, t = new llOO1(id, edge, installed_modes[gamemode]), map = [...new Array(size)].map(i=> new Array(size).fill(0)), f = {};
        for (let i = -edge; i < edge; i++) {
            for (let j = -edge; j < edge; j++) {
                t.get(i,j,f);
                if (f.on) map[Math.trunc(edge-f.y-0.1)][Math.trunc(f.x+edge)] = Math.max(Math.min(Math.round(f.size*100/6),1),1);
            }
        }
        return map;
    }else{
...
module.exports.createMapByID = createMapByID;